...
export const DV1001: Rule = {
  ...
  check(ctx) {
      ...
      for (const inst of stage.instructions) {
        if (inst.type === 'ENV') {
          ...
          for (const pair of e.pairs) {
            if (SECRET_PATTERNS.test(pair.key) && pair.value && pair.value !== '' && !pair.value.startsWith('$')) {
        ...
        if (inst.type === 'ARG') {
          ...
          if (SECRET_PATTERNS.test(a.name) && a.defaultValue && a.defaultValue !== '' && !a.defaultValue.startsWith('$')) {
    ...
    for (const arg of ctx.ast.globalArgs) {
      if (SECRET_PATTERNS.test(arg.name) && arg.defaultValue && arg.defaultValue !== '' && !arg.defaultValue.startsWith('$')) {
